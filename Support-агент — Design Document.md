...
| NFR6 | Плавная деградация | RAG confidence < τ_rag → эскалация; сбой LLM API → retry ×3 с exponential backoff (≤ 10 сек суммарно), затем эскалация с причиной `llm_unavailable` |
...
- Given релевантных документов не найдено (score < τ_rag для всех), When RAG-поиск завершён, Then RAG confidence считается ниже порога, что триггерит соответствующую ветку decision table (R6/R8)

**UC4 - автоответ**

- Given тикет категории FAQ с RAG confidence ≥ τ_rag, When агент формирует ответ, Then ответ отправляется клиенту за ≤ 8 сек (NFR1), взаимодействие полностью залогировано
- Given тикет категории FAQ, но RAG confidence < τ_rag, When агент пытается ответить, Then происходит эскалация оператору с пометкой "low RAG confidence"
...
- Actor: Агент
- Precondition: Категория FAQ (R4), RAG confidence ≥ τ_rag, класс. confidence ≥ 0.85. Статус заказа до интеграции с системой заказов автоответа не получает (R10)
- Main flow:
...
| R7c | техпроблема | ≥ τ_rag | < 0.85 | 0 | нет | A2 |
| R7a | техпроблема | ≥ τ_rag | ≥ 0.85 | 0 | нет | A3 |
| R8 | техпроблема | < τ_rag | любой | 0 | нет | A2 |
| R5 | FAQ / статус заказа | ≥ τ_rag | < 0.85 | любая | нет | A2 |
| R6 | FAQ / статус заказа | < τ_rag | любой | любая | нет | A2 |
| R4 | FAQ / статус заказа | ≥ τ_rag | ≥ 0.85 | любая | нет | A1 |
| R-default | любая другая комбинация | - | - | - | - | A2 + алерт `decision_table_gap` |

τ_rag - калибруемый порог RAG confidence: сейчас 0.60 под bge-m3 в гибридном режиме, происхождение значения - в «Confidence и пороги». Литерал в таблице намеренно не пишется: значение меняется вместе с embedding-моделью, а правила - нет.
...
- **R7c - закрыта дыра.** Комбинация «техпроблема, 0-я итерация, RAG ≥ τ_rag, класс. confidence < 0.85» не покрывалась ни одним правилом: R7a требовал confidence ≥ 0.85, R8 - RAG < τ_rag. Поведение выровнено с UC2: низкий класс. confidence всегда ведёт к эскалации без попытки автообработки.
...
Два разных числа в decision table называются «confidence», и их природа различается - без этого различия пороги 0.85 и τ_rag выглядят взятыми с потолка.
...
**Калибровка.** Пороги - калибруемые значения, а не константы домена; исходные 0.85 и 0.7 были стартовыми. Процедура: на golden set строятся кривые precision/recall по порогу; порог для high-risk выбирается как минимальный, при котором recall ≥ 0.95 (NFR2); порог RAG - точка, в которой доля автоответов «без опоры на найденный документ» падает ниже 5% по ручной разметке 100 ответов. Пороги вынесены в конфиг, а не в код правил, и версионируются. Смена embedding-модели или LLM-провайдера **требует** повторной калибровки - косинусная шкала не переносится между моделями; это зафиксировано как обязательный шаг в ADR-009 и ADR-010. Для k-sampling отдельно: шкала дискретна с шагом 1/k, поэтому порог, откалиброванный на logprobs, к ней не переносится.

**Калибровка τ_rag под bge-m3** (`scripts.calibrate_rag`, отчёт `eval/rag_calibration_BAAI_bge-m3.json`). Ручной разметки нет, опора размечена автоматически: обращение из golden set опирается на выдачу, если среди top-5 есть ожидаемый документ. Golden set для этой задачи оказался непригоден сам по себе: у всех содержательных обращений в нём есть ответ в базе знаний, и любой порог выглядел бы безопасным. Поэтому добавлен набор `eval/out_of_kb.jsonl` - 36 вопросов на темы, которых в базе нет (подарочные сертификаты, рассрочка, международная доставка и т. п.); автоответ по ним - всегда выдумка. На 172 обращениях faq / tech_issue из golden set и 36 вопросах вне базы:

| τ_rag | доля автоответов без опоры | вопросов вне базы проходит | покрытие вопросов с ответом в базе |
| --- | --- | --- | --- |
| 0.55 | 9.0% | 8 из 36 | 98.8% |
| 0.57 | 4.8% | 4 | 97.5% |
| **0.60** | **3.2%** | **1** | **93.2%** |
| 0.70 (исходное) | 1.5% | 0 | 39.3% |

Процедура даёт 0.57, но там доля лежит вплотную к 5% при всего 36 негативных примерах - принято 0.60 с запасом: ошибки несимметричны, выдуманный автоответ хуже лишней эскалации. Исходное 0.7, записанное как «откалиброванное под bge-m3», на деле ни разу не проверялось и отправляло к оператору 60% вопросов, на которые в базе есть ответ. Для хеширующего провайдера (локальная разработка, CI) шкала своя, порог ~0.15.
...
NFR2 выполнен. Модель logprobs не отдаёт, и k-sampling оказался бесполезен: 357 из 359 обращений получили уверенность 1.0 (пять голосов из пяти), в том числе 34 из 35 ошибочных. Порог 0.85 ничего не отсекал, а единственный ошибочный автоответ - «Здравствуйте» → `faq` с уверенностью 5/5. Решение - ADR-012. Ограничения замера: RAG шёл на хеширующих векторах (Recall@5 = 0.73 не зависит от LLM), задержка провайдера под нагрузкой не проверялась (NFR1, NFR8). Стоимость измерена отдельно (`scripts.measure_cost`): 41 обращение через полный граф агента с генерацией - p95 0.108 ₽ ≈ $0.0013 на тикет при пороге NFR5 $0.02; эскалация без черновика стоит ~0.03 ₽ (одна классификация), автоответ и черновик - ~0.1 ₽. Без нагрузки p95 обработки тикета - 1.9 с при бюджете NFR1 8 с. Попутная находка: через локальный HTTP-прокси установка TLS-соединения с провайдером занимала до 50 с при 0.3 с на сам ответ - соединения с провайдером должны быть прямыми и переиспользоваться (общий пул в адаптере).
...
| Векторы | Режим | Recall@5 |
| --- | --- | --- |
| хеширующие | только вектор | 0.73 |
| хеширующие | гибрид, текст документа | 0.79 |
| хеширующие | гибрид, текст + заголовок | 0.83 |
| bge-m3 | только вектор | 0.89 |
| **bge-m3** | **гибрид, текст + заголовок (принято)** | **0.92** |
...
*Итог.* С bge-m3 (ADR-006) гибрид даёт 0.92 - цель достигнута, и гибрид добавляет +3 п.п. даже поверх сильных векторов: полнотекстовая ветка ловит точные слова, которые эмбеддинг размывает. На хеширующих векторах потолок - 0.83: ранжирование Postgres (`ts_rank_cd`) не учитывает редкость слова (IDF, как в BM25, нет), и частое «заказ» перетягивает выдачу. Отсечение слов, встречающихся в большой доле документов, давало там до 0.85, но порог пришлось бы подбирать на том же golden set, на котором идёт замер, - отклонено как подгонка под тест. С переходом на bge-m3 порог RAG перекалиброван: 0.60 вместо 0.7 (см. «Confidence и пороги»). Trade-off: заголовок складывается в запросе, а не хранится в версии, - смена заголовка видна поиску сразу, но для заголовка GIN-индекс не используется; на базе знаний в сотни и тысячи документов это незаметно, дальше - денормализовать заголовок в индексируемую колонку.
...
| Unit: пороги и confidence | Расчёт `rag_confidence` (max по top-k), нормализация `class_confidence`, поведение на границах порогов | граничные значения проверены явно (τ_rag ± 0.0001, 0.8499 / 0.85) |
...
| Стоимость на тикет | ≤ $0.02 (NFR5) | превышение дневного бюджета. Источник - `llm_usage` в `audit_log`: вызовы, токены и стоимость из ответов провайдера на каждый тикет |
...
*Реализация.* Метрики - эндпоинт `/metrics` (Prometheus), `app/metrics.py`. Разделены по природе: **события** - счётчики в коде (`support_tickets_processed_total{action, rule_id}`, гистограмма `support_ticket_processing_seconds`, `support_escalations_total{reason}`, расход LLM, `support_operator_actions_total`), доли и скорости считает PromQL; **состояние** - снимается при опросе из первоисточника (возраст и число неопубликованных событий outbox из Postgres, глубина `escalations.created` и DLX из RabbitMQ). Счётчик состояния в воркере соврал бы: вставший поллер перестаёт обновлять и свои метрики, а алерт нужен именно тогда. Алерты по каждой строке таблицы - `deploy/prometheus/alerts.yml`, с юнит-тестами `promtool` на синтетических рядах (включая отрицательные случаи: малый трафик, `for` ещё не истёк) и проверкой в CI. Коридор доли автоответов (5–40%) и порог стоимости (1.68 ₽ ≈ $0.02) взяты из замеров golden set и NFR5 - стартовые значения, пересматриваются по реальному трафику. Runbook - `docs/runbook.md`, по разделу на алерт, ссылка в аннотации `runbook_url`. Prometheus поднимается профилем `docker compose --profile observability`. `trace_id` берётся из заголовка `X-Trace-Id` (только hex/UUID: произвольная строка из заголовка была бы инъекцией в логи) или генерируется, возвращается в заголовке ответа и в теле любой ошибки, пишется в `audit_log`, в событие outbox и в логи consumer'а. Логи - JSON в API и воркерах; PII-редакция применяется ко всей строке, включая трейсбек.

*Что нашлось при реализации.* Эскалация при недоступности LLM создавалась в обход агента - без записи в `audit_log` (нарушение NFR3) и без учёта в метриках, хотя SLI `llm_unavailable` - именно про неё; исправлено. Ошибки валидации (422) и роутинга (404) уходили в формате FastAPI по умолчанию, без кода и `trace_id`, - теперь в единой модели ошибок.
...
| Качество embedding недостаточно на реальных RU-тикетах (сленг, домен) | Средняя | Среднее | Гибрид на bge-m3 (ADR-010): Recall@5 = 0.92 на golden set; оценка на размеченной выборке реальных тикетов перед запуском - golden set синтетический |
...
| Пороги RAG и класса не переносятся при смене embedding-модели или LLM-провайдера | Высокая - **подтверждено**: τ_rag под bge-m3 оказался 0.60, а не 0.7 | Среднее | Пороги в конфиге; калибровка скриптом `scripts.calibrate_rag` на golden set + вопросах вне базы; обязательная рекалибровка как шаг ADR-009 / ADR-010 |
