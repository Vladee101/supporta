...
| NFR4 | Безопасность и приватность | PII замаскирован перед вызовом LLM и в логах; сырые тикеты удаляются retention-джобом ≤ 90 дней; клиент не может прочитать чужой тикет (подписанный `ticket_token`, см. API-контракты) |
...
3. **logprobs - желательный, а не отсекающий критерий.** Если провайдер их не отдаёт, `class_confidence` - доля голосов k-sampling (k = 5), и пятикратная стоимость классификации закладывается в NFR5.
...
*Trade-off.* У k-sampling две цены. Первая - стоимость ×k. Вторая - дискретная шкала: при k = 5 уверенность принимает только значения 0; 0,2; …; 1,0, и порог 0,85 фактически означает «все пять голосов за одну категорию». Для провайдера без logprobs порог калибруется отдельно, а не переносится. Плюс два диалекта вместо одного и обязательная рекалибровка порогов при смене любой модели. Ни один адаптер пока не вызывался на живом ключе: контракт GigaChat v2 сверен по официальному клиенту `ai-forever/gigachat`, OpenAI-совместимый - по документации YandexGPT и vLLM.

**ADR-010. Чанкинг KB и гибридный retrieval.** Решение: документы режутся на чанки 300–500 токенов с перекрытием 15% по границам заголовков; в MVP retrieval чисто векторный, но схема и интерфейс ретривера с самого начала рассчитаны на гибрид (BM25 через `tsvector` + вектор, слияние по RRF), включение - конфигом, без миграции. Обоснование: на русских support-тикетах много точных токенов (номера заказов, коды ошибок, названия тарифов), по которым lexical-поиск сильнее эмбеддингов; при этом ставить гибрид в MVP без замера - преждевременная оптимизация. Триггер включения задан заранее: Recall@5 ниже 0.9 на golden set (см. «Методика оценки»). Trade-off: `tsvector`-индекс и вторая ветка ранжирования содержатся в схеме, даже пока не используются.
...
Отдельно: тесты на PII-редакцию (в LLM не уходит неотредактированный текст) и на retention-джоб (тикеты старше 90 дней удаляются, `audit_log` и агрегаты сохраняются).
...
| Качество embedding недостаточно на реальных RU-тикетах (сленг, домен) | Средняя | Среднее | Оценка на размеченной выборке перед запуском; fallback на гибридный retrieval (keyword + vector) |
...
| Провайдер не отдаёт logprobs, и class_confidence считается k-sampling с дискретной шкалой | Высокая | Среднее (NFR2, NFR5) | Режим `auto` в адаптере; отдельная калибровка порога под k; стоимость ×k заложена в NFR5 (ADR-009) |
