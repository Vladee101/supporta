...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "category IN ('faq', 'order_status', 'complaint', 'refund', 'tech_issue', 'unclassified')",
            name="ck_category_category",
        ),
        sa.CheckConstraint(
            "risk_level IN ('low', 'medium', 'high')", name="ck_risk_level_risklevel"
        ),
        sa.CheckConstraint(
            "status IN ('new', 'classified', 'awaiting_clarification', 'escalated_standard', 'escalated_priority', 'pending_operator', 'in_progress', 'resolved_auto', 'resolved_by_operator')",
            name="ck_status_ticketstatus",
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_confidence_source_confidencesource",
        ),
        sa.CheckConstraint(
            "risk_level IN ('low', 'medium', 'high')", name="ck_risk_level_risklevel"
        ),
    ...
    op.create_table(
        ...
        sa.Column("resolved_at", sa.DateTime(timezone=True), nullable=True),
        sa.CheckConstraint(
            "reason IN ('client_requested', 'high_risk_category', 'classification_failed', 'low_class_confidence', 'low_rag_confidence', 'clarification_limit_reached', 'clarification_timeout', 'agent_timeout', 'llm_unavailable', 'decision_table_gap')",
            name="ck_reason_escalationreason",
        ),
        sa.CheckConstraint(
            "status IN ('pending', 'in_progress', 'resolved')", name="ck_status_escalationstatus"
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "sender IN ('client', 'agent', 'operator')", name="ck_sender_messagesender"
        ),
        sa.ForeignKeyConstraint(["ticket_id"], ["tickets.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "action_type IN ('confirm', 'edit', 'reject')", name="ck_action_type_operatoractiontype"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["document_version_id"], ["kb_document_versions.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(["ticket_id"], ["tickets.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "ix_audit_log_ticket_created_at", "audit_log", ["ticket_id", "created_at"], unique=False
    )
    op.create_index(
        "ix_escalations_queue", "escalations", ["status", "priority", "created_at"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_messages_ticket_created_at", "messages", ["ticket_id", "created_at"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_rag_retrievals_ticket_iteration",
        "rag_retrievals",
        ["ticket_id", "iteration"],
        unique=False,
    )
