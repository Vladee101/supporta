...
Эскалация в работе: переписка, черновик ответа агента, классификация с источником
уверенности и документы, которые нашёл поиск, - с версией, которую видел агент (ADR-011).

![Карточка эскалации](docs/screenshots/escalation.png)

Очередь - по приоритету и времени ожидания; правило decision table и наличие черновика
видны сразу. Администратор ведёт базу знаний с историей версий.

![Очередь эскалаций](docs/screenshots/queue.png)

![База знаний](docs/screenshots/knowledge-base.png)

Снимки сделаны на демо-стенде (`docker compose --profile demo`) - на базовой линии:
словарный классификатор и шаблонные черновики.

## Результаты

Отчёт по NFR собирается скриптом из артефактов прогонов, числа руками не вносятся -
[reports/nfr_report.md](reports/nfr_report.md). Итог: **10 из 10 выполнено**.
...
| NFR5: стоимость на тикет | ≤ $0.02 | p95 $0.0013 (0.11 ₽): 41 тикет через полный граф на Alice AI Flash |
| Задержка реального провайдера, без нагрузки | NFR1: ≤ 8 с | p95 тикета 1.9 с, автоответы ≤ 2.0 с |
...
Нужен только Docker. Весь стенд - Postgres, RabbitMQ, API, воркеры и консоль оператора -
в контейнерах, без ключей и платных вызовов (словарный классификатор, шаблонные ответы):

```bash
docker compose --profile demo up -d --build
```

Через минуту-две (первая сборка образа дольше) - ссылка для входа в консоль:

```bash
docker compose --profile demo logs demo
```

Ссылка вида `http://localhost:8000/console/#token=...` открывает консоль под
администратором: очередь с эскалациями из демо-обращений, их контекст и журнал решений,
база знаний. Порт меняется переменной `DEMO_PORT`. Остановить - `docker compose --profile
demo down` (с `-v` - вместе с данными). CI поднимает этот стенд с нуля на каждом push и
проверяет, что эскалации прошли outbox → RabbitMQ → consumer.
...
| - | Демо-стенд одной командой (`docker compose --profile demo`), проверка в CI | готово |
