...
| Гибридный поиск по базе знаний | Включён по триггеру из ADR-010 (Recall@5 < 0.9): полнотекстовый поиск Postgres с русской морфологией + вектор, слияние по RRF; Recall@5 0.73 → 0.83 | ADR-010, [retrieval.py](app/services/retrieval.py) |
...
перекалибровать по этому файлу офлайн, не повторяя платные вызовы. RAG во всех
прогонах идёт на хеширующих векторах - веса bge-m3 весят ~2 ГБ и не скачаны,
поэтому Recall@5 от выбора LLM не зависит. Поиск гибридный (ADR-010): векторный
режим давал 0.73, гибрид - 0.83; до порога 0.9 не хватает, основной резерв -
замер с bge-m3.
